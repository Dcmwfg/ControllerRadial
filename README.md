...
10. **任何界面都能用手柄鼠标**（Controlify 自带：左摇杆光标 / A 左键 / X 右键 / 右摇杆滚动 / 方向键跳控件）+ 本 mod 补的**拖拽**（RS 按下按住左键），地图平移、模组界面拖动都能用；地图另有可选专用绑定（默认不绑）。
...
2. 用途：地图平移、模组界面里的拖拽（拖滑块/拖物品之类）。任何界面都生效。
3. 它通过调用界面的 `mouseClicked / mouseDragged / mouseReleased` 实现，所以不依赖 MC 内部的鼠标状态。

### 地图（Xaero）里怎么用

1. **左摇杆** → 移动光标（Controlify 自带，约 20 px/tick）——在地图上悬停会显示坐标。
2. **右摇杆上下** → 缩放（Controlify 自带的真实滚轮事件）。
3. **RT（右扳机）** → **鼠标右键**（本 mod 新增，默认已绑）：开右键菜单/新建路径点；发出右键时那一帧会**取消 Controlify 的滚动**，所以不会边开菜单边缩放。
4. **方向键** → 光标微调一格 16 px（本 mod 新增，默认已绑，按住每 4 tick 连发）——用来精确挪到菜单项，然后用 **A** 点。
5. **RS 按住 + 左摇杆** → **拖拽平移地图**（本 mod 的通用拖拽绑定）。

另外三个绑定（`地图：移动光标（摇杆）` / `地图：放大` / `地图：缩小`）**默认不绑**——因为 Controlify 自带的左摇杆光标与右摇杆滚动已经覆盖了，再绑会变成双倍速度。想换成别的键自己绑即可。

> 1.18.0 曾把地图绑定全部设为默认不绑，结果 RT/D-Pad 就没反应了（玩家反馈“地图功能不行”）；1.19.0 把 **RT 右键 + D-Pad 微调** 改回默认绑定。
>
